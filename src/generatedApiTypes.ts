...
export interface components {
  schemas: {
     ...
     * Event is no longer one object. It is EventDevice | EventEdge, discriminated on `source`.
     * Any caller that reads `event.identification` (or any other device-only field) without
     * narrowing on `source` fails TypeScript. Runtime JSON parsing is unchanged.
     * Hits getEvent, searchEvents, and unsealEventsResponse. Webhook bodies without `source`
     * also fail `as Event` (see tests/mocked-responses-tests/castWebhookEventTest.spec.ts).
     */
